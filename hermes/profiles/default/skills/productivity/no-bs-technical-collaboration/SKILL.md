...
- **Polymarket py-clob-client-v2: `OrderType.IOC` does not exist.** Available types are FAK, FOK, GTC, GTD. FAK (Fill-and-Kill) is the correct choice for market sells. Using IOC will cause silent failures or crashes. Always verify with `dir(OrderType)` before using order types.
- **Polymarket-specific pitfalls** (Docker --no-cache, crypto engine stuck trades, VPN rebuild, backend/frontend filter mismatch, dashboard P/L formula consistency, stop-loss mechanics, resolution bugs, entry price verification, order types, etc.) now live in `polymarket-debugging-patterns/` and `polymarket-order-types/` references — load `no-bs-technical-collaboration` skill for the full list.
- **When user asks about wallet/money, lead with the LIVE answer — never bury it in paper context.** Keith has multiple strategies running: some paper-only, some live. When he asks "what happened to my wallet" or "how much did I lose," the FIRST thing out of your mouth must be the real-money answer. If direction is 0/76 paper losses but momentum is +$1.80 live, lead with "Momentum: +$1.80. Direction is paper-only so those 76 losses are fake." Do NOT lead with "Direction lost 76 trades" — that buries the actual answer under noise. Paper is supporting context, never the headline. This was triggered when the agent answered "all losses" and led with directional paper stats, causing Keith to think paper trades drained his real wallet.

- **Polymarket is the source of truth for ALL data — not the DB.** This goes beyond wallet balance. When there is ANY discrepancy between what the DB says and what Polymarket's APIs say, Polymarket wins. Always. This covers: **entry prices** (actual fill price from FAK order response, never the assumed ask at decision time), **resolution outcomes** (Gamma API `outcomePrices` are authoritative over Chainlink/Binance BTC data), **wallet balances**, and **filled vs unfilled orders**. The DB is a mirror — if it reflects something different from Polymarket, the DB is wrong and must be fixed. Keith has said this explicitly: "Don't assume in the DB. Whatever logic you have that makes your DB as a source of truth is completely wrong. We have to follow whatever is in the real life data."

- **When Keith pushes back because you're repeating things he already knows, stop explaining and start acting.** If he says "I know paper doesn't affect my wallet, why are you explaining that?" — he's not confused. He's telling you to skip the recap and move to the next diagnostic step. The correct response is an immediate deeper investigation (on-chain forensics, API queries, etc.), not a re-explanation of concepts he already understands.

- **Distinguish "engine activity" from "external events" when wallet money disappears.** When Keith asks where his money went, first confirm what the engine did (trades placed, costs incurred), then separately state what the engine did NOT do (withdrawals, transfers). This prevents the false impression that paper strategies or small live trades explain a large balance drop. If the engine only accounts for a small fraction of the missing funds, say so upfront: "The engine only moved $X. $Y is unaccounted for and must be external."

- **When private keys are suspected compromised, run the full security audit.** See `references/private-key-security-audit.md` for the checklist: git history, log scan, Docker log scan, file permissions, SSH access, and key transmission path tracing. Also covers the critical distinction between a single-key leak (one chain affected) and a seed phrase compromise (all chains affected).
