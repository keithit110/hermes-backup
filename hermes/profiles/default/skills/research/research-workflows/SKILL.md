...
For read-only Polymarket intelligence systems, use the pattern in `references/polymarket-intel-scanner.md`: containerized market scanner, arbitrage detector, smart-wallet tracker, information monitor, paper-trading ledger, and Hermes cron wrapper. Start read-only/paper-only; do not add private keys or live trading until signals are proven and explicit execution guardrails exist. For Keith's current workflow, prefer ≤3-day markets, keep single-wallet copy and smart-wallet consensus as separate paper-trade strategies unless he explicitly asks to remove one, and use clear final-result-sync labels (`closed_pending_final_result_sync`) rather than implying manual intervention.

For news/research-driven Polymarket paper trades, follow the bet-first workflow in `references/polymarket-bet-first-research.md`: discover tradable near-resolution bets first, generate a small candidate list with exact token IDs, and research only those candidates. Do not broadly scan unrelated news feeds before markets exist; if no candidates are available, write `IGNORE` and spend no web/LLM research tokens. Do not let sports dominate by default: Keith wants politics/geopolitics, macro/regulatory, tech/AI/business, weather/events, and other news-driven markets surfaced when tradable candidates exist. Crypto is intentionally excluded from Keith's current research buckets unless he explicitly asks to restore it.
