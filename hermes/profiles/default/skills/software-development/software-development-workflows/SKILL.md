...
- `references/live-trading-engine-verification.md` — use when building live Polymarket trading engines with py-clob-client-v2; covers FAK vs FOK order types, minimum order sizes, GTC phantom fills, stale resolution prices, entry price guards, scanner skip lists, live-only strategy patterns, fill-verification pattern (Polymarket as source of truth), trend detection from own P/L data, dynamic position sizing, and dashboard wallet balance grounding.
- `references/gtc-vs-fak-phantom-fills.md` — use when GTC orders placed at ask price don't fill (phantom trades in DB, wallet balance unchanged); covers FAK-vs-GTC selection, P/L discrepancy diagnosis, and phantom trade cleanup.
