...
### CRITICAL: `"once"` vs `"every"` — cron schedule traps

`"once in 15m"` is a ONE-SHOT delay — it fires 15 minutes from now, ONE time only. After completion, `state: completed, enabled: false`. The job never runs again. This is a recurring cause of trades sitting open for hours/days because the scanner/close loop runs once then stops forever.

`"every 15m"` is recurring — it fires every 15 minutes forever. This is what you want for any daemon, scanner, health check, or periodic task.

**Always verify with `cronjob(action='list')`**: check `state` (should be `active`, not `completed`) and `enabled` (should be `true`). If a recurring job shows `state: completed, enabled: false`, it was created with `"once"` — delete and recreate with `"every"`.

**Also check `next_run`**: a completed one-shot job has no next_run. A healthy recurring job has a next_run in the future.
